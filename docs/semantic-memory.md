...
During workflow execution, the engine queries the memory storage using cosine similarity. It compares the current execution context against stored embeddings. A configurable similarity threshold filter is applied to prevent irrelevant noise from being injected into the agent's prompt.
